...
exports.validateSelected = async function(){
    ...
    if(current.type === 'microsoft') {
        ...
        // Conta offline — sempre válida, não precisa de validação online
        return true
    }
}
 ...
 * The account is stored locally with a UUID generated from the username.
 *
 * @param {string} username The desired username (3-16 chars, letters/numbers/_).
 * @returns {Object} The created auth account object.
 */
exports.addOfflineAccount = function(username) {
    // Gera um UUID determinístico baseado no username
    const paddedHex = username.toLowerCase()
        .split('')
    ...
    const uuidClean = ('00000000000000000000' + paddedHex).slice(-32)

    const ret = ConfigManager.addMojangAuthAccount(
        uuidClean,
        'offline_token_' + username,
        username,
        username
    )
    ConfigManager.save()
